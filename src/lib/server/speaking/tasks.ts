...
export async function getOrCreateSpeakingTask(
  userId: number,
  appId: AppId,
): Promise<{ task: SpeakingTask; existing: boolean; needsDelivery: boolean }> {
  ...
  if (existing) {
    ...
    return {
      ...
      existing: true,
      needsDelivery: existing.status === "ready",
    };
  }
  ...
  if (items.length === 0) {
    throw new AppError(
      "NO_PRACTICING_ITEMS",
      "No phrases are ready for speaking practice yet. Complete a quiz or move a phrase to Practicing in the App.",
      409,
    );
  }
