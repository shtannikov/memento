...
- Make the scenario coherent as a whole, not just a collection of individually plausible details. The setting, mission, requested information, and intended outcome must form a realistic causal chain. Before finalizing it, mentally play the scene from start to finish and track the learner's location, each movement, the order of events, what the learner can know, and who takes each action. Anchor relative places and objects to the specific location they belong to. Every transition must be physically and causally plausible without the learner inventing missing facts.
...
- coherentScenario: the setting, mission, requested details, and intended outcome form one realistic causal chain rather than a collage of unrelated ideas. Locations, movements, temporal references, and causes remain compatible from sentence to sentence; relative places and objects are anchored to a specific location; the learner is only asked to relate events they witnessed or were plausibly told about; and the grammar focus does not introduce facts that the setup never establishes.
- groundedSequenceAndReferences: independently trace the scene moment by moment. Set this to false if any movement lacks a reason, any place or object lacks a clear spatial anchor, any requested event is outside the learner's established knowledge, or any participant reference could match more than one person. Do not let the scene's overall plausibility compensate for one unresolved gap.
