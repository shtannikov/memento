...
- Give the learner a believable reason to take the requested action. Do not make the learner speak, choose, order, collect, or decide on behalf of another capable adult merely because they are together or know each other. If the learner must act as that person's proxy, state the concrete reason the other person cannot reasonably act for themselves, such as a language barrier, a lost voice, or being elsewhere. Otherwise, let the person concerned act directly or give the learner their own stake or responsibility in the mission.
...
- Keep the scene warm, vivid, and conversational. Target 220–270 characters and never exceed 280, leaving safe room below the application's 300-character validation limit.
...
- Before returning the task, reject the draft if any requested past fact is absent from the setup.
...
- groundedSequence: independently trace the scene moment by moment. Set this to false if a mission-relevant movement lacks a reason, a place or object required to understand the task lacks a clear spatial anchor, or a requested event is outside the learner's established knowledge. Do not let overall plausibility compensate for a gap that makes the setup, requested response, or intended outcome contradictory. Do not fail for omitted incidental details that the learner does not need to complete the task.
...
- justifiedLearnerRole: the learner has a believable personal stake, responsibility, or reason to take the requested action. Set this to false when the learner speaks, chooses, orders, collects, or decides for another capable adult without the setup explaining why that person cannot reasonably act for themselves. Being together, being friends or colleagues, or the other person's having the problem does not by itself justify the learner acting as their proxy.
