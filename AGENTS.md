...
- Required server-only Vercel variables are `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, `SUPABASE_URL`, `SUPABASE_SECRET_KEY`, `OPENAI_API_KEY`, and `OPENAI_CHAT_MODEL`.
- Point the Stage bot at the active pull request Preview with `npm run telegram:webhook:set -- https://<preview-host>`. Point the Production bot at the stable production URL after deploying `main`. Run the command with that environment's `TELEGRAM_BOT_TOKEN` and `TELEGRAM_WEBHOOK_SECRET`; never reuse Production credentials in Preview.
