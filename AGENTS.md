...
- OpenAI schemas, model configuration, and generation validation live in `src/server/openai.ts`; each language's prompts, grader, starter vocabulary, manifest, and evals live together under `src/languages/<app-id>/` and are consumed by that production path.
...
- GitHub's `Stage` environment must contain the five Stage-only Telegram secrets used by the post-deployment configuration workflow: `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, `TELEGRAM_CZ_BOT_TOKEN`, `TELEGRAM_CZ_WEBHOOK_SECRET`, and `TELEGRAM_ADMIN_BOT_TOKEN`. Do not copy Production bot credentials into GitHub Stage.
...
- Required server-only Vercel variables are `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, `TELEGRAM_CZ_BOT_TOKEN`, `TELEGRAM_CZ_WEBHOOK_SECRET`, `TELEGRAM_ADMIN_BOT_TOKEN`, `SUPABASE_URL`, `SUPABASE_SECRET_KEY`, `OPENAI_API_KEY`, `OPENAI_CHAT_MODEL`, `OPENAI_SPEAKING_EVALUATION_MODEL`, `OPENAI_STT_MODEL`, and `CRON_SECRET`. Set `OPENAI_STT_MODEL` to `gpt-4o-transcribe`.
