...
create function memento.admin_list_user_app_stats(
  ...
  speaking_completed_today bigint,
...
as $$
  select
    ...
    coalesce(quizzes.completed_today, 0) as quizzes_completed_today,
    quizzes.last_completed_at as last_quiz_completed_at,
    coalesce(speaking.completed, 0) as speaking_completed,
    coalesce(speaking.completed_today, 0) as speaking_completed_today,
  ...
  left join lateral (
    select
      count(*) as completed,
      count(*) filter (
        where round.completed_at >= requested_date::timestamp at time zone 'utc'
          and round.completed_at < (requested_date + 1)::timestamp at time zone 'utc'
      ) as completed_today,
  ...
  left join lateral (
    select
      count(*) as completed,
      count(*) filter (
        where task.completed_at >= requested_date::timestamp at time zone 'utc'
          and task.completed_at < (requested_date + 1)::timestamp at time zone 'utc'
      ) as completed_today,
